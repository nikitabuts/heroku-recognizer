...
class DataGenerator:
    def __init__(self, a_len, b_len, alpha_len, random_comand="randn", alpha_initialize="base_circle"):
        ...
        self.x = np.arange(-50, 51, 0.01)
        self.y = self.x

    def fig_plot(self, function, ax=True, save=False, plot=True, verbose=500):       #Если save=True - то укажите свой PATH --> код ниже
        counter = 1700
        var_x = self.x.copy()
        var_y = self.y.copy()
