...
class Learn:
    def __init__(self, epochs, train_loader, pic_size=64 * 64 * 4, test_loader=None):
        self.net = Net()
        self.epochs = epochs
    ...
    def _accuracy(model, loader):
        ...
        with torch.no_grad():
            for data in loader:
                X_, y_ = data
                output = model(X_.view(-1, 64 * 64 * 4))
    ...
    def fit(self, use_best_model=True, print_flag=True):
        ...
        for epoch in range(self.epochs):
            for data in self.train_loader:
                ...
                loss.backward()
                self.optimizer.step()
            train_accuracy = self._accuracy(self.net, self.train_loader)

            if self.test_loader != None:
                test_accuracy = self._accuracy(self.net, self.test_loader)
            ...
            if print_flag:
                ...
                print('')
        self.scores_frame = scores_frame.copy()

        return self.net, self.scores_frame

    def get_best_model(self):
        PATH = "C:\\Torch\\torch_" + str(self.scores_frame['Test_Accuracy'].argmax() + 1) + ".pt"
        model = Net()
    ...
    def accuracy_by_classes(self, net, loader,
        ...
        class_dict = {0: np.zeros(5), 1: np.zeros(5), 2: np.zeros(5), 3: np.zeros(5), 4: np.zeros(5)}

        for data in loader:
            X, y = data
            output = net(X.view(-1, 64 * 64 * 4))
            for idx, i in enumerate(output):
        ...
        for class_ in classes:
            print(
                "   {} accuracy: {}".format(dictionary[class_], np.round((correct_mass[class_] / all_mass[class_]), 3)))
        print("------------------------")
